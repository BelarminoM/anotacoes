...
# $interface => Índice da interface que será consultada
...
for ((t=0; t<601; t+=3)); do
	entrada=`snmpget -v2c -c $leitura $router ifInOctets.$interface | cut -d: -f4`
	saida=`snmpget -v2c -c $leitura $router ifOutOctets.$interface | cut -d: -f4`
